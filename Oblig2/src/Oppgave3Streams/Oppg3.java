...
//		3 a) 
		...
		List<String> etternavnene = ansatter.stream().map((a) -> a.getEtternavn()).collect(Collectors.toList());
		utskriftAlle(etternavnene);

//		3 b) 
		System.out.println("b) Finn ut antall kvinner blant de ansatte.");
		int antallKvinner = (int) ansatter.stream().filter((a) -> a.getKjonn().equals(Kjonn.KVINNE)).count();
		System.out.println(antallKvinner + "\n");

//		3 c) 
		System.out.println("c) Regn ut gjennomsnittslønnen til kvinnene.");
		double gjennomsnittslønnen = ansatter.stream().filter(a -> a.getKjonn().equals(Kjonn.KVINNE))
				.mapToDouble(a -> a.getAarslonn()).average().orElse(0);
...
//		double prosentTilegg = 07.0; // a.setAarslonn((int) (a.getAarslonn() * (1 + (prosentTilegg / 100))))
		ansatter.stream().filter(a -> a.getStilling().contains("sjef"))
...
//		3 e) 
		...
		boolean ansattSomTjenerMerEnn800000 = ansatter.stream().anyMatch(a -> a.getAarslonn() > 800000);
...
//		3 g) 
		...
		Optional<Ansatt> ansatt = ansatter.stream()
...
//		3 h) 
		...
		int sum = IntStream.range(1, 1000).filter(a -> a % 3 == 0 || a % 5 == 0).sum();
